<?php
    ...
    function costered_rebuild_post_css($postId) {
        ...
        if (function_exists('do_blocks')) {
            $renderedContent = do_blocks($post->post_content);
        } else {
            // Fallback for very old WordPress where blocks do not exist.
            $renderedContent = $post->post_content;
        }
    ...
    function costered_rebuild_all_styles_from_things(): int {
        ...
        * rewriting the query into something less clear or less correct. See note at top. */
        ...
        $rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
            $wpdb->prepare(
                "SELECT thing_costered_id, thing_key, thing_data FROM " . COSTERED_THINGS_TABLE . " WHERE thing_type = %s",
                'style'
            ),
            ARRAY_A
        );
    ...
    function costered_sync_styles_for_post(int $post_id): int {
        ...
        $walker = function (array $block_list) use (&$walker, $table, &$written_count, $wpdb) {
            foreach ($block_list as $block) {
                ...
                $has_costered = isset($attrs['costered']) && is_array($attrs['costered']);
                        ...
                        foreach (array('desktop', 'tablet', 'mobile') as $breakpoint) {
                            ...
                            $existing_id = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
                                $wpdb->prepare( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
                                    "SELECT id FROM " . COSTERED_THINGS_TABLE . " WHERE thing_key = %s AND thing_costered_id = %s LIMIT 1",
                                    $breakpoint,
                                    $uid
                                )
                            );
                ...
                // Always recurse into inner blocks, even if this block had no costered data or no UID.
