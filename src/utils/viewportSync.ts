...
export function startViewportSync(): () => void {
    ...
    const update = () => {
        ...
        if (bp !== lastBp) {
            ...
            if (window.CB_DEBUG_TOOLS_ACTIVE) console.log('[costered:bp-sync]', { dev, bp }); //TODO: change this to a Costered specific debug function so we can omit the if statement.
        }
    };
